...
public class FireFluidSim : MonoBehaviour 
{
	...
	//You can change this but you must change the same vaule in all the compute shader's to the same
	...
	void Start () 
	{
		//Dimenstion sizes must be pow2 numbers
		m_width = Mathf.ClosestPowerOfTwo(m_width);
		m_height = Mathf.ClosestPowerOfTwo(m_height);
		m_depth = Mathf.ClosestPowerOfTwo(m_depth);
		
		//Put all dimesion sizes in a vector for easy parsing to shader and also prevents user changing
		...
		//At the moment is only the border around the edge of the buffers to enforce non-slip boundry conditions
		ComputeObstacles();
	...
	void ComputeObstacles()
	{
		...
		m_computeObstacles.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ApplyImpulse(float dt, float amount, ComputeBuffer[] buffer)
	{
		...
		m_applyImpulse.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ApplyExtinguishmentImpulse(float dt, float amount, ComputeBuffer[] buffer)
	{
		...
		m_applyImpulse.Dispatch(1, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ApplyBuoyancy(float dt)
	{
		...
		m_applyBuoyancy.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ApplyAdvection(float dt, float dissipation, float decay, ComputeBuffer[] buffer, float forward = 1.0f)
	{
		...
		m_applyAdvect.Dispatch((int)ADVECTION.NORMAL, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ApplyAdvectionBFECC(float dt, float dissipation, float decay, ComputeBuffer[] buffer)
	{
		...
		m_applyAdvect.Dispatch((int)ADVECTION.BFECC, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ApplyAdvectionMacCormack(float dt, float dissipation, float decay, ComputeBuffer[] buffer)
	{
		...
		m_applyAdvect.Dispatch((int)ADVECTION.MACCORMACK, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ApplyAdvectionVelocity(float dt)
	{
		...
		m_applyAdvect.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ComputeVorticityConfinement(float dt)
	{
		...
		m_computeVorticity.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
		...
		m_computeConfinement.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ComputeDivergence()
	{
		...
		m_computeDivergence.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	}
	...
	void ComputePressure()
	{
		...
		for(int i = 0; i < m_iterations; i++)
		{
			...
			m_computeJacobi.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void ComputeProjection()
	{
		...
		m_computeProjection.Dispatch(0, (int)m_size.x/NUM_TREADS, (int)m_size.y/NUM_TREADS, (int)m_size.z/NUM_TREADS);
	...
	void Update () 
	{
		...
		//First off advect any buffers that contain physical quatities like density or temperature by the 
		//velocity field. Advection is what moves quanties around.
		...
		//BFECC is a method of advection that helps to prevents this smoothing at a extra performace cost but is less numerically stable.
		//MacCormack does the same as BFECC but is more (not completely) numerically stable and is more costly
		//You only really need to do this type of advection on visible fields, but you can do it on non visibles ones if you want
		...
		//Apply the effect the sinking colder smoke has on the velcoity field
		ApplyBuoyancy(dt);
		...
		//fliuid is models as being incompressable meaning that the volume of the fluid
		//does not change over time. The divergence is the amount the field has deviated from being divergence free
		ComputeDivergence();
		
		//This computes the presure need return the fluid to a divergence free condition
		ComputePressure();
		...
		renderer.material.SetVector("_Translate", transform.localPosition);
		renderer.material.SetVector("_Scale", transform.localScale);
		renderer.material.SetBuffer("_Density", m_density[READ]);
		renderer.material.SetBuffer("_Reaction", m_reaction[READ]);
		renderer.material.SetVector("_Size", m_size);
